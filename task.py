...
class ServerTask(Task, ABC):
    ...
    def initialize(self) -> None:
        ...
        if self.connection_mode == ConnectionMode.MULTI_NETWORK_DENY:
            self._create_multi_network_policy("deny", "ingress", self.port)
            self._create_multi_network_policy("deny", "egress", self.port)
        elif self.connection_mode == ConnectionMode.MULTI_NETWORK_ALLOW:
            self._create_multi_network_policy("allow", "ingress", self.port)
            self._create_multi_network_policy("allow", "egress", self.port)
